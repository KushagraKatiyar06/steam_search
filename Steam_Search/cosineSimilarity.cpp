...
void cosineSimilarity::createGameSignatures(const unordered_map<string, Game>& allData) {
    ...
    //each index represents a specific tag. The value at the index is (number of votes for particular tag)/ (total votes for every tag the game has).
    for (auto const& game : allData) {
        ...
        //Populating the vector with the proportion of votes that a particular tag has relative to all votes for every tag in the game
        for (auto tag : currentGame.getTags()) {
...
double cosineSimilarity:: similarity(const string& gameA, const string& gameB) {
    ...
    //returns normalized similarituy score
